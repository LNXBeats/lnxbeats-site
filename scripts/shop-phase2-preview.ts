...
async function run() {
  ...
  console.info(operation === "build"
    ? "Building the guarded Shop Phase 2 local preview."
    : `Starting the guarded Shop Phase 2 local preview at ${SHOP_PHASE2_QA_ORIGIN}.`);
...
run().catch(() => {
  console.error("The guarded Shop Phase 2 local preview command failed.");
  process.exitCode = 1;
});
