...
const ALLOWED_ENV = [
  ...
  "SHOP_SHIPPING_PROVIDER_ENABLED", "SHOP_SHIPPING_PROVIDER", "SHOP_SHIPPING_PROVIDER_QA_CONFIRM",
...
async function guard() {
  ...
  assert.equal(process.env.AUTH_URL, SHOP_SHIPPING_PROVIDER_QA_ORIGIN);
...
async function run() {
  ...
  const args = operation === "build" ? [nextCli, "build", "--webpack"] : [nextCli, "start", "-H", "127.0.0.1", "-p", "31779"];
  const env = {
...
run().catch((error: unknown) => {
  console.error("The guarded Phase 5D preview command failed.");
