...
import {
  ...
  SHOP_SHIPPING_OPERATIONS_QA_ORIGIN,
...
const ALLOWED_ENV = [
  ...
  "SHOP_SHIPPING_OPERATIONS_ENABLED", "SHOP_SHIPPING_OPERATIONS_QA_CONFIRM", "SHOP_SHIPPING_OPERATIONS_PROVIDER",
...
async function guard() {
  ...
  assert.equal(process.env.AUTH_URL, SHOP_SHIPPING_OPERATIONS_QA_ORIGIN);
...
async function run() {
  ...
  const args = operation === "build" ? [nextCli, "build", "--webpack"] : [nextCli, "start", "-H", "127.0.0.1", "-p", "31777"];
  const env = {
    ...Object.fromEntries(ALLOWED_ENV.flatMap((name) => process.env[name] === undefined ? [] : [[name, process.env[name]!]])),
    NODE_ENV: process.env.NODE_ENV ?? "test",
  } as NodeJS.ProcessEnv;
  console.info(operation === "build" ? "Building guarded Phase 5C preview." : `Starting guarded Phase 5C preview at ${SHOP_SHIPPING_OPERATIONS_QA_ORIGIN}.`);
...
run().catch((error: unknown) => {
  console.error("The guarded Phase 5C preview command failed.");
