...
Le candidat Railway devra prouver avant activation : SHA attendu, image contenant Prisma, Pre-deploy `npx prisma migrate deploy`, Start `npm start`, healthcheck `/api/health`, 17 migrations, paiements/notifications désarmés et scripts de diagnostic présents.
