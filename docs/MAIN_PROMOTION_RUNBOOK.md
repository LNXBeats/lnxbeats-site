...
Le candidat Railway devra prouver avant activation : SHA attendu, image contenant Prisma, Pre-deploy `npx prisma migrate deploy`, Start `npm start`, healthcheck `/api/health`, nombre réel de migrations (28 après la fondation locale Phase 5E), paiements/notifications/Boutique désarmés et scripts de diagnostic présents.
...
Pour V1.1.0, appliquer également les gates documentés dans [SHOP_PRODUCTION_READINESS_PHASE5E.md](SHOP_PRODUCTION_READINESS_PHASE5E.md). Cette documentation locale n'autorise aucune promotion ni activation.
