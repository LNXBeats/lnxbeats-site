...
- `PRODUCTION BLOCKED — LIVE REFUND ACTIVATION REQUIRES A DEDICATED SPRINT` : cette limite est indépendante de Stripe Checkout Live. La première phase Checkout impose `LIVE_REFUNDS_ENABLED=false`; l’activation future des remboursements Live exige la correction et la validation du retry ambigu, d’un plafond/cutoff et d’une réconciliation provider sûre. Les parcours Stripe Test et PayPal Sandbox restent disponibles pour la QA financière.
