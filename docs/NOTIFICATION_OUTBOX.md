...
Une erreur temporaire devient `FAILED_RETRYABLE`, avec backoff 5 min, 30 min, 2 h puis 6 h. Après cinq claims ou une erreur finale : `FAILED_FINAL`. Les retours provider peuvent produire `BOUNCED`, `COMPLAINED` ou `SUPPRESSED`. `CANCELED` est réservé à une invalidation explicite future.

Le claim est sérialisé par verrou transactionnel PostgreSQL et compare le statut/version avant de poser une lease de cinq minutes. Une lease expirée récente peut être reprise. Deux workers ne peuvent pas réclamer simultanément la même ligne. Le provider reçoit la clé d’idempotence persistante. [Resend conserve cette garantie pendant 24 heures](https://resend.com/docs/dashboard/emails/idempotency-keys) ; LNX arrête volontairement tout retry automatique ambigu après 12 heures depuis le premier claim : la ligne devient `FAILED_FINAL` avec `AMBIGUOUS_PROVIDER_ACCEPTANCE` et exige une réconciliation humaine.
