...
Une ShopOrder payée non expédiée peut recevoir une demande MEMBER d'annulation ou de correction d'adresse. Le MEMBER ne mute jamais lui-même la commande, le paiement, le stock ou la facture. L'Admin revalide sous le verrou partagé de la commande. Une annulation payée exige d'abord la facture source compatible avec la même commande et le paiement gagnant ; une absence ou incohérence s'arrête avant tentative et provider. La transaction courte de réservation crée ensuite une barrière persistante qui interdit préparation, suivi et expédition incompatibles pendant que l'appel provider s'exécute hors transaction. Un intent transport déjà parti bloque réciproquement l'annulation. Après succès financier, le restock, l'avoir au motif précis d'annulation client et les états de commande sont idempotents et exécutés exactement une fois. Si le provider a accepté mais que la finalisation locale échoue, la même tentative reste réservée en `REQUIRES_REVIEW` avec sa preuve connue ; seule une réconciliation de cette tentative peut terminer le workflow. Une correction d'adresse ne recalcule ni total ni shipping et ne réécrit jamais la facture historique. Toute demande après expédition est refusée.
...
La preuve locale Phase 5E part d'une instance Prisma Dev jetable vide, applique les 29 migrations, conserve les snapshots historiques couverts par PGlite, ferme la Boutique par flags lors du rollback applicatif et rejoue les invariants sur PostgreSQL loopback. Elle ne copie aucune donnée réelle.
