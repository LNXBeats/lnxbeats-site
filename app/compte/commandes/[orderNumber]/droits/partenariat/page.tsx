...
export default async function ExploitationPartnershipPage({ params }: { params: Promise<{ orderNumber: string }> }) {
  ...
  const session = await requireVerifiedUser(`/compte/commandes/${orderNumber}/droits/partenariat`);
  ...
  return <section className="auth-shell rights-shell"><Container><RightsRequestForm type="EXPLOITATION_PARTNERSHIP" orderNumber={order.orderNumber} orderTitle={order.title || order.recipient || "Création LNX"} account={{ firstName, lastName: lastNameParts.join(" "), artistName: session.user.name, email: session.user.email }} /></Container></section>;
}
