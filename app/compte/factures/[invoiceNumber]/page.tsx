...
export default async function MemberInvoicePage({ params }: { params: Promise<{ invoiceNumber: string }> }) {
  ...
  return <section className="auth-shell account-shell billing-document-page"><Container className="auth-shell__inner auth-shell__inner--account">
    <div className="auth-intro"><p className="eyebrow">{presentation.label}</p><h1 className="technical-reference technical-reference--hero">{invoice.invoiceNumber}</h1><p>Facture liée à la commande <span className="technical-reference technical-reference--inline">{invoice.orderNumberSnapshot}</span>. Le PDF est généré depuis le snapshot immuable enregistré au paiement.</p></div>
    <section className="auth-panel"><dl className="auth-profile"><div><dt>Émise le</dt><dd>{invoice.issuedAt.toLocaleString("fr-FR")}</dd></div><div><dt>Montant</dt><dd>{formatEuro(invoice.totalCents)}</dd></div><div><dt>Paiement</dt><dd>{invoice.paymentMethodLabel}</dd></div><div><dt>TVA</dt><dd>{invoice.vatLegalNotice}</dd></div><div><dt>Commande</dt><dd>{invoice.orderNumberSnapshot}</dd></div><div><dt>Profil de facturation</dt><dd>{customer.type === "PROFESSIONAL" ? "Professionnel" : "Particulier"}</dd></div><div><dt>Destinataire</dt><dd>{customer.companyName || customer.name}{customer.companyName ? <small>{customer.name}</small> : null}</dd></div>{customer.billingAddress ? <div><dt>Adresse de facturation</dt><dd><address className="billing-customer-address">{billingAddressLines(customer.billingAddress).map((line, index) => <span key={`${index}:${line}`}>{line}</span>)}</address></dd></div> : null}{customer.businessIdentifier ? <div><dt>SIREN / SIRET</dt><dd>{customer.businessIdentifier}</dd></div> : null}{customer.vatId ? <div><dt>Identifiant TVA client</dt><dd>{customer.vatId}</dd></div> : null}<div><dt>Empreinte</dt><dd><code>{invoice.snapshotHashSha256.slice(0, 16)}…</code></dd></div></dl>
