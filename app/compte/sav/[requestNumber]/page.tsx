...
const STATUS = {
  REQUESTED: "Demande enregistrée", UNDER_REVIEW: "En cours d’examen", APPROVED: "Acceptée",
  REJECTED: "Refusée", AWAITING_RETURN: "Retour attendu", RETURN_RECEIVED: "Retour reçu",
  INSPECTED: "Inspection terminée", REFUND_PENDING: "Remboursement en cours", REFUNDED: "Remboursée",
...
export default async function MemberShopReturnPage({ params, searchParams }: { params: Promise<{ requestNumber: string }>; searchParams: Promise<{ etat?: string }> }) {
  ...
  return <div className="auth-shell account-shell"><Container className="auth-shell__inner auth-shell__inner--account">
    ...
    <header className="auth-intro"><p className="eyebrow">Boutique · SAV</p><h1>{request.requestNumber}</h1><div className="account-intro__summary"><p>{STATUS[request.status]}. Les décisions de remboursement et de remise en stock restent séparées et auditées.</p></div></header>
    ...
    <div className="auth-account-stack">
      <section className="member-orders"><div className="member-orders__heading"><div><p className="auth-panel__label">État</p><h2>{STATUS[request.status]}</h2></div></div><dl className="auth-profile">
        ...
        <div><dt>Remboursement</dt><dd>{request.refundStatus}</dd></div>
