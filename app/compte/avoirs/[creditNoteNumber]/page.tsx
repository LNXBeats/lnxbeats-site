...
export default async function MemberCreditNotePage({ params }: { params: Promise<{ creditNoteNumber: string }> }) {
  ...
  return <section className="auth-shell account-shell billing-document-page"><Container className="auth-shell__inner auth-shell__inner--account">
    <div className="auth-intro billing-credit-note-intro"><p className="eyebrow">{presentation.label}</p><h1 className="technical-reference technical-reference--hero">{creditNote.creditNoteNumber}</h1><p>Correction de la facture <span className="technical-reference technical-reference--inline">{creditNote.invoice.invoiceNumber}</span>, sans suppression ni réécriture du document d’origine.</p></div>
