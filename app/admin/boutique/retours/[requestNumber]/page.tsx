...
export default async function AdminShopReturnPage({ params, searchParams }: { params: Promise<{ requestNumber: string }>; searchParams: Promise<{ etat?: string }> }) {
  ...
  const refundProvider = shopAfterSalesRefundProvider();
  ...
  return <div className="admin-main">
    ...
    <div className="admin-order-detail__grid"><div className="admin-order-detail__main">
      ...
      {mayRefund && !request.refundAttempt ? refundProvider === "disabled" || refundProvider === "blocked" ? <section className="admin-side-window"><p className="admin-section-label">Remboursement</p><p><strong>Remboursements financiers désarmés.</strong></p><p>La décision SAV reste enregistrée, mais aucun appel au prestataire n’est autorisé.</p></section> : <section className="admin-side-window"><p className="admin-section-label">{refundProvider === "fake" ? "Remboursement QA factice" : "Remboursement financier"}</p><form className="admin-form" action={refundShopReturnAction}><input type="hidden" name="requestNumber" value={request.requestNumber} /><label>Expédition remboursée<select name="shippingDecision" defaultValue="NONE"><option value="NONE">Non</option><option value="FULL">Oui, intégralement</option></select></label>{refundProvider === "fake" ? <label>Scénario provider factice<select name="behavior" defaultValue="SUCCEEDED"><option value="SUCCEEDED">Succès</option><option value="PENDING">Pending</option><option value="FAILED">Échec</option><option value="AMBIGUOUS">Timeout ambigu</option></select></label> : null}<Confirmation value={SHOP_RETURN_REFUND_CONFIRMATION}>{refundProvider === "fake" ? "Je confirme le remboursement QA calculé par le serveur." : "Je confirme ce remboursement réel calculé par le serveur auprès du prestataire du paiement."}</Confirmation><button className="admin-button" type="submit">{refundProvider === "fake" ? "EXÉCUTER LE FAKE REFUND" : "CONFIRMER LE REMBOURSEMENT"}</button></form></section> : null}
      {request.refundAttempt && ["PENDING", "REQUIRES_REVIEW"].includes(request.refundAttempt.status) ? <section className="admin-side-window"><p className="admin-section-label">Réconciliation</p>{refundProvider === "disabled" || refundProvider === "blocked" ? <p><strong>Réconciliation provider désarmée — revue humaine requise.</strong></p> : <form action={reconcileShopReturnAction}><input type="hidden" name="requestNumber" value={request.requestNumber} /><button className="admin-button" type="submit">{refundProvider === "fake" ? "RÉCONCILIER LE FAKE REFUND" : "VÉRIFIER AUPRÈS DU PRESTATAIRE"}</button></form>}</section> : null}
