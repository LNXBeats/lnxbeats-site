...
export default async function AdminCatalogueEditPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ etat?: string }> }) {
  ...
  return <div className="admin-main admin-catalogue-editor">
    ...
    <section className="admin-detail-window">
      ...
      {cover ? <div className="admin-cover-preview"><Image src={`/media/catalog/${cover.id}`} alt={coverAlt} width={320} height={320} sizes="240px" /><div><p>Cover officielle actuelle</p><Link className="admin-row-action" href={`/album/${project.slug}`} target="_blank" rel="noreferrer">Voir sur le site <ExternalLinkIcon /></Link></div></div> : <p className="admin-muted">Aucune cover officielle. L’espace graphique de repli reste visible publiquement.</p>}
