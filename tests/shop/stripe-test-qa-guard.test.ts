...
import { assertShopPhase3BStripeQaEnvironment } from "@/lib/shop/stripe-test-qa-guard";
...
const environment = {
  ...
  STRIPE_MODE: "test",
  STRIPE_SECRET_KEY: "sk_test_phase3b_fixture",
  STRIPE_WEBHOOK_SECRET: "whsec_phase3b_fixture",
...
test("Shop Phase 3B accepts only the armed loopback Stripe TEST runtime", async () => {
  const result = await assertShopPhase3BStripeQaEnvironment(environment, proof);
  assert.equal(result.stripeMode, "test");
  assert.equal(result.target, SHOP_PHASE2_QA_TARGET);
});

test("Shop Phase 3B refuses Stripe LIVE, PayPal credentials, remote DBs and missing armament", async () => {
  for (const override of [
    { STRIPE_SECRET_KEY: "sk_live_forbidden_fixture" },
    ...
    await assert.rejects(() => assertShopPhase3BStripeQaEnvironment(
...
test("historical Stripe reconciliation remains allowed after only the Shop Checkout kill switch closes", async () => {
  await assertShopPhase3BStripeQaEnvironment(
